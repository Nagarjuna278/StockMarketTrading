...
public:
    ...
    void addOrder(const Order& order) {
        // Simple lock mechanism for transaction safety
        while (inTransaction) {
...
void simulateTransactions(int numTransactions = 10) {
    for (int i = 0; i < numTransactions; i++) {
        ...
        TickerString ticker = tickers[rng.randInt(0, NUM_TICKERS - 1)];
...
// Simulate one broker's activity
void simulateBroker(int brokerId, int iterations) {
...
void runSimulation() {
    printf("Starting stock exchange simulation...\n");
    
    // Initialize order books and tickers
    initOrderBooks();
    initTickers();
    
    // Simulate 5 brokers sequentially
    for (int i = 0; i < 1000; i++) {
        // Simulate each broker's activity
        for (int broker = 0; broker < 5; broker++) {
            simulateBroker(broker, 1);
        }
        
        // Print progress every 100 iterations
        if (i % 100 == 0) {
            printf("Simulation progress: %d%%\n", i / 10);
        }
    }
